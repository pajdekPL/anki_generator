...
class GenankiVocabHandler:
    ...
    MODELS = {'vocab_simple_model': genanki.Model(
        MODEL_ID,
    ...
    def __init__(self, deck_name: str, model: str = "vocab_simple_model") -> None:
        ...
        if model in GenankiVocabHandler.MODELS:
            ...
            raise ErrorModelDoesNotExist(f"Model: {model} does not exist, available models: "
    ...
    def add_note_vocab_simple_model(self, word: str, meanings: [str],
                                    examples: [str], ipa: [str], pronunciation_mp3: [str]) -> None:
        new_note = genanki.Note(
            model=self.model,
            fields=[word, f'[sound:{pronunciation_mp3}]', "<br>".join(meanings),
                    "<br>".join(examples), ipa])
        self.deck.add_note(new_note)

    def add_vocab_flashcard_to_deck(self, media_file_path: Path, word: str, meanings: [str],
                                    examples: [str], ipa: [str], media_file_name: [str]) -> None:
